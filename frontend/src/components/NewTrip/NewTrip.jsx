          ...
          detail
            ...
            <>
              <div>
                ...
                <input id="search" type="text" value={input.search} onChange={(e) => handleInput(e, setInput)} />
                <button className={styles.btn} type="button" onClick={() => setData(findPlace)}>Search</button>
              </div>
              <AddTripInputField destinations={destinations} resetFunc={setDestinations} />
              <SearchResults />
            </>
        }
      </div>
          ...
          return (
            <li key={index} className={styles.searchResult} >
              <div onClick={() => navigateToDetail(place.place_id)}>
                <h3><u><b>{place.name}</b></u></h3>
              </div>
              <p>{place.formatted_address}</p>
              <p>Rate: {place.rating} &#40;{place.user_ratings_total}&#41;</p>
              <button className={styles.add} onClick={() => addTrip(place.name, place.place_id)}>add trip</button>
            </li>
          )
        })}
      </ul>
